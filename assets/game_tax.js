...
window.TaxGame = (function(){
  ...
  /* 잘 피하는 봇으로 재 보고 정한 값이다(2026-08-31 실측).
     ...
     🔴🔴 2026-09-02 석봉님 "게임 난이도가 더 어려워져야 할 것 같아, **하다가 지루함**을 느꼈다."
        ...
        START 1.45 → 시작 속도 150→218, 시작 간격 1.25→0.86초. 첫 5초부터 손이 바쁘다.
        상한도 520 → 600 으로 올려 뒤가 더 조인다.
        📏 **봇 중앙값으로는 이 변화가 안 잡힌다**(21.8초 → 17.0초, 큰 차이가 아니다).
           봇은 완벽하게 피하니 **초반이 심심한 것을 못 느낀다.** 사람이 느끼는 지루함은
           「할 게 없는 것」이므로 **초반 밀도**를 세는 편이 옳다 —
             10초까지 떨어지는 개수  9개 → **14개**
             20초까지               23개 → **41개**
           (다음에 난이도를 만질 때도 봇 기록만 보지 말고 이 개수를 함께 볼 것)
        (고칠 자리: **앞이 지루하면 START, 뒤가 싱거우면 PACE**) */
  var PACE = 1.95;
  var START = 1.45;
