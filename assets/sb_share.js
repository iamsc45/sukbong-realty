 ...
 *
 * 준비 (2026-08-31 확인)
 *   ① JavaScript 키 — 받았다(아래 KEY)
 *   ② 🔴 **[플랫폼 키] > [JavaScript 키] > [JavaScript SDK 도메인]** 에 도메인 등록이 따로 필요하다.
 *      「플랫폼 > Web > 사이트 도메인」과 **다른 설정**이다(카카오 콘솔 안내문에 그렇게 적혀 있다).
 *      등록 안 되어 있으면 공유 호출이 실패하지만, 아래 any() 가 시스템 공유로 물러서므로 화면은 안 죽는다.
 *   ③ ~~제품 설정 > 카카오톡 공유 활성화~~ → **그런 설정은 없다.**
 *      카카오톡 공유는 켜고 끄는 스위치가 없다(카카오 로그인과 다르다).
 *      공식 문서(카카오톡 공유 > 이해하기·JavaScript)에 「사전 설정」·「활성화」 항목이 아예 없음을 확인했다.
 *   ⚠️ 카카오톡 공유창은 **모바일에서 카카오톡 앱**이 뜬다. PC 브라우저에서는 동작이 다르다.
 *
...
window.SBShare = (function(){
  ...
  var KEY = "756c107acdb10488d3791efa9d9ddc42";   // 카카오 JavaScript 키 (2026-08-31 등록)
